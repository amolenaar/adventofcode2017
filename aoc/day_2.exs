...
3858	202	1141	3458	2507	239	199	4400	3713	3980	4170	227	3968	1688	4352	4168
  ...
  def lines() do
    ...
    |> String.trim()
    |> String.split("\n")
    |> Enum.map(fn line -> String.split(line, ~r/\s/, trim: true)
                           |> Enum.map(&String.to_integer/1) end)
  end
  

  def difference(line), do: Enum.max(line) - Enum.min(line)
  ...
  def divisable_counterpart(a, []), do: :none
  def divisable_counterpart(a, [b | rest]) do
    cond do
      rem(a, b) == 0 -> {:ok, a, b}
      rem(b, a) == 0 -> {:ok, b, a}
      true -> divisable_counterpart(a, rest)
    end
  end

  def divisable([a | rest]) do
    case divisable_counterpart(a, rest) do
      {:ok, a, b} -> a / b
      _ -> divisable(rest)
    end
  end
